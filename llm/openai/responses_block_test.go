...
// copies produce pointer forms) is skipped by the Chat Completions adapter —
// never an error, never emitted — while the other blocks are preserved.
func TestConvertMessagesSkipsPointerReasoningItem(t *testing.T) {
	...
	// The reasoning item is skipped; the text survives as content.
