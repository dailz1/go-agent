...
└── tool/       # Tool iface, soft-error ToolResult, RWMutex Registry
```

## WHERE TO LOOK
| Task | Location | Notes |
|------|----------|-------|
| Understand the loop | agent `runStreamInternal` (agent.go:304) | single execution path; `Run` folds the stream |
| Add a provider adapter | llm/glm or llm/openai | copy an adapter; update BOTH (see llm/AGENTS.md) |
| Change Message/Chunk/Event types | docs/DESIGN.md, then llm | sealed interfaces; DESIGN.md is doc of record |
| Add a tool | tool/tool.go | implement `Tool`, register via `Registry` |
| Tune context management | agent compact.go + truncate.go | compaction at 80% of window; tool results ~30% |
...
|--------|------|----------|------|------|
| llm.Provider | interface | llm/provider.go:19 | agent + both adapters | Name/Chat/ChatStream; ErrStreamingNotSupported sentinel |
| llm.Message / ContentBlock | struct / sealed iface | llm/message.go | highest traffic | custom UnmarshalJSON (string/array/null) |
| llm.Chunk | sealed iface | llm/chunk.go | heavy in agent | 6 stream variants |
| agent.Agent | struct | agent/agent.go:100 | consumer entry | loop, approval, retry, compaction wiring |
| agent.New | func | agent/agent.go:211 | | central normalization of all options |
| agent.Compactor | interface | agent/compact.go:43 | | 4 strategies + chain |
| tool.Tool / tool.Registry | iface / struct | tool/tool.go:14, registry.go | ~20 | soft-error contract; thread-safe |
| llm.APIError | struct | llm/errors.go | retry path | StatusCode/RetryAfter/Retryable(); leaf, no Unwrap |
...
- Test seams are package vars: `sleepFor` (llm/retry.go), `timeNow` (llm/glm/auth.go).
