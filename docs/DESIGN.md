...
- 会话持久化 `Store`：**内核自动持久化**（`WithStore` 选项 + `RunThread` / `ResumeThread` / `RunThreadStream` 线程入口；未配置零开销：不生成线程 ID、不触碰持久化路径，单一执行路径不变）。数据形状：线程 = 规范记录日志，生命周期记录链每条带稳定 `run_id` / `round_id`：
  ...
  - 检查点：可重建加速器，定位在其自身记录的 Seq；快照解码或校验失败 → 经 `History(ctx, thread, fromSeq)` 全量回退重建（`Latest` 只返回尾部，故必须提供范围读）。日志全量保留，v1 不清理。
...
- 提交点与流式栅栏：**输入先落库再调模型；宣告先落库再执行任何工具；Done 先落库再发出**。流式事件先落库再 yield 给消费者；消费者中途断开 → 运行保持未完成（可恢复）；持久化失败即终止本轮并返回存储错误（按序合并原始错误，默认安全）。
- 并发：每线程非阻塞运行所有权——第二个并发运行在**任何模型调用之前**即得类型化冲突（单纯加锁等待是串行化，不是冲突）；乐观版本号追加 + record_id 幂等重试作为其他写者的后盾。不可变记录批次与 ID 只生成一次，重试逐字节同一内容。
- 线程 ID：调用方 ID 非空、有界、不规范化（JSONL 经 PathEscape 成文件名）；内核生成 = crypto-random 128 位，经首个 rev=0 追加保留，仅在该次保留冲突时重试；`RunResult` 与 `DoneEvent` 暴露 `ThreadID`；无 store 时不生成。head=0 即"新建"，无"线程不存在"语义；调用方 ID 复用即"追加到既有线程"。`RunWithHistory` / `RunStreamWithHistory` 保持非持久（一次性导入语义，不入线程）。
- v1 单进程（写入按线程串行化；同目录仅允许一个 JSONL 实例，由 NewJSONL 强制）。v1 非目标：HITL 等待落库、日志清理、跨进程协调；同步审批回调参与宣告/提交轮状态，但不是持久化 HITL 等待。
